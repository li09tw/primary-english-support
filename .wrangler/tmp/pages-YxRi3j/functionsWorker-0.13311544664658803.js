...
async function onRequest(context) {
  ...
  try {
    ...
    const result = await db.prepare("SELECT * FROM game_methods ORDER BY created_at DESC").all();
    ...
    if (category) {
      filteredGames = filteredGames.filter(
        (game) => game.categories.includes(category)
      );
    }
    if (grade) {
      filteredGames = filteredGames.filter(
        (game) => game.grades.includes(grade)
      );
    }
...
__name(convertGradesToArray, "convertGradesToArray");
...
// ../.wrangler/tmp/pages-YxRi3j/functionsRoutes-0.4082980647569989.mjs
var routes = [
  {
    routePath: "/games",
    mountPath: "/",

...
export default function Header() {
  ...
  const navigation = [
    ...
    { name: '管理介面', href: '/admin' },
  ];

  return (
    <header className="bg-gradient-to-r from-rose-300 via-pink-300 to-purple-300 shadow-lg">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex-shrink-0">
            <Link href="/" className="flex items-center space-x-2">
              <div className="w-10 h-10 bg-gradient-to-br from-pink-400 to-purple-500 rounded-lg flex items-center justify-center">
                <span className="text-white font-bold text-lg">PES</span>
              </div>
              <span className="text-xl font-bold text-gray-800">PrimaryEnglishSupport</span>
            </Link>
          </div>
        ...
        {isMenuOpen && (
          <div className="md:hidden">
            <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3 bg-white/80 backdrop-blur-sm rounded-lg mt-2">
              {navigation.map((item) => (
                <Link
                  key={item.name}
                  href={item.href}
